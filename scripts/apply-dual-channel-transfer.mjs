...
const connectMarker = "    const client = TcpSocket.createConnection(\n      {host: discoveredHost.ip, port: discoveredHost.port},";
if (!source.includes(connectMarker)) throw new Error('Patch failed: node connect marker');
replaceOnce('transfer client connect', connectMarker, `    const transferClient = TcpSocket.createConnection(\n      {host: discoveredHost.ip, port: TRANSFER_PORT},\n      () => {\n        transferClientRef.current = transferClient;\n        addLog('Connected audio transfer channel');\n      },\n    );\n\n    transferClient.on('data', data => {\n      (transferClient as unknown as PartySocketBuffer)._partyBuffer = \`${'${'}(transferClient as unknown as PartySocketBuffer)._partyBuffer || ''}${'${'}data.toString()}\`;\n      const lines = ((transferClient as unknown as PartySocketBuffer)._partyBuffer || '').split('\\n');\n      (transferClient as unknown as PartySocketBuffer)._partyBuffer = lines.pop() || '';\n      lines.forEach((message: string) => {\n        if (!message.trim()) return;\n        handleHostMessage(message, transferClient);\n      });\n    });\n    transferClient.on('error', error => addLog(\`Transfer connection error: ${'${'}String(error)}\`));\n    transferClient.on('close', () => { transferClientRef.current = null; });\n\n${connectMarker}`);

replaceOnce('handler signature', '  const handleHostMessage = async (message: string) => {', '  const handleHostMessage = async (message: string, responseSocket: any = clientRef.current) => {');
replaceOnce('chunk ack', "          if (index % TRANSFER_BATCH_SIZE === 0 || index === buffer.chunks.length - 1) {\n            const percent", "          if ((index + 1) % TRANSFER_ACK_EVERY_CHUNKS === 0 || index === buffer.chunks.length - 1) {\n            writeSocket(responseSocket, `TRANSFER_ACK|${trackId}|${index}`);\n          }\n\n          if (index % TRANSFER_BATCH_SIZE === 0 || index === buffer.chunks.length - 1) {\n            const percent");
replaceOnce('track received control ack', "            writeSocket(clientRef.current, `TRACK_RECEIVED|${trackId}|${buffer.name}`);", "            writeSocket(clientRef.current, `TRACK_RECEIVED|${trackId}|${buffer.name}`);");
